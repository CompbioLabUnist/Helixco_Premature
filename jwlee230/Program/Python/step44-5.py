"""
step44-5.py: make DESeq2 input coldata for True/False data
"""
import argparse
import pandas
import tqdm
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compare", help="Compare clinical data", default="Mother Antibiotics")
    ...
    metadata = metadata.loc[(metadata["Premature"] == "PTB")]
    print(metadata)
    ...
    for site in tqdm.tqdm(step00.selected_long_sites):
        ...
        index = list(filter(lambda x: (x in set(tmp_data.loc[~(tmp_data[args.compare])].index)), list(input_data.index))) + list(filter(lambda x: (x in set(tmp_data.loc[(tmp_data[args.compare])].index)), list(input_data.index)))

        output_data = input_data.loc[index, :]
        output_data.T.to_csv(f"{args.output}.{site}.tsv", sep="\t")

        output_data = pandas.DataFrame(index=index)
        output_data["condition"] = list(map(lambda x: metadata.loc[x, args.compare], list(output_data.index)))
