...
if __name__ == "__main__":
    ...
    parser.add_argument("taxonomy", help="Taxonomy TSV file", type=str)
    ...
    elif not args.taxonomy.endswith(".tsv"):
        raise ValueError("Taxonomy must end with .TSV!!")
    ...
    taxonomy_data = pandas.read_csv(args.taxonomy, sep="\t", index_col=0)
    print(taxonomy_data)

    input_data["taxonomy"] = list(map(lambda x: taxonomy_data.loc[x, "taxonomy"], list(input_data.index)))
    input_data = input_data.loc[list(map(step00.filtering_taxonomy, input_data["taxonomy"])), ]
    input_data["simple_name"] = list(map(step00.consistency_taxonomy, input_data["taxonomy"]))
    print(input_data)
    ...
    down_results = input_data.loc[((input_data["logFC"] < numpy.log2(1 / ratio_threshold)) & (input_data["-log10(p)"] > (-1 * numpy.log10(p_threshold)))), :].sort_values("-log10(p)", ascending=False)
    up_results = input_data.loc[((input_data["logFC"] > numpy.log2(ratio_threshold)) & (input_data["-log10(p)"] > (-1 * numpy.log10(p_threshold)))), :].sort_values("-log10(p)", ascending=False)
