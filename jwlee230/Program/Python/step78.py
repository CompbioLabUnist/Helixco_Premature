"""
step78.py: Gaussian process Classifier with DAT-union
"""
import argparse
import itertools
import tarfile
...
if __name__ == "__main__":
    ...
    classifier = sklearn.gaussian_process.GaussianProcessClassifier(sklearn.gaussian_process.kernels.RBF(1.0), random_state=42, multi_class="one_vs_one", n_jobs=args.cpus)
    ...
    std = numpy.std(input_data[taxa].to_numpy(), axis=0)
    best_features = list(map(lambda x: x[1], sorted(zip(std, taxa), reverse=True)))
    ...
    seaborn.histplot(data=std, stat="count", kde=True, ax=ax)

    matplotlib.pyplot.xlabel("STD")
    matplotlib.pyplot.title("PTB vs. Normal")
    matplotlib.pyplot.tight_layout()

    tar_files.append("stds.pdf")
