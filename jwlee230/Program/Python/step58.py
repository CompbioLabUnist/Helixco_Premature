"""
step58.py: RandomForest Classifier with Bagging
"""
import argparse
import itertools
import tarfile
    ...
    for site in tqdm.tqdm(step00.selected_long_sites):
        ...
        for i in tqdm.trange(1, len(best_features)):
            for j, (train_index, test_index) in enumerate(k_fold.split(tmp_data[best_features[:i]], tmp_data[target])):
                ...
                for metric in step00.derivations:
                    try:
                        test_scores.append((i, metric, step00.aggregate_confusion_matrix(numpy.sum(sklearn.metrics.multilabel_confusion_matrix(y_test, bagging_classifier.predict(x_test)), axis=0), metric)))
                    except ZeroDivisionError:
                        continue
